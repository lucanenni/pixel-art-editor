...
## v1.2.6 - Compact QR encoding, eyedropper snaps to the current palette

- **Bug fix**: a fully-painted 8x8 drawing (any palette) could already exceed the QR code's ~2000-character capacity and fall back to the "too large to share" message — even though 8x8 is the *smallest* grid size, which was confusing ("try a smaller grid" doesn't help when you're already on the smallest one). The cause: each pixel's color was stored as a full `"rgb(r,g,b)"` string (up to 18 characters), far more than needed for a palette of only 16/64/256 possible colors.
- Reworked the QR payload to store each pixel's **palette index** (a small number, e.g. `7` instead of `"rgb(170,170,170)"`) instead of the color string, via new `encodePixelsCompact()`/`decodePixelsCompact()` helpers in `logic.js`. For the exact reported case (8x8, fully painted, CGA), this cut the share URL from ~1998 characters down to ~770 — comfortably under the limit, with a lot of headroom to spare. Falls back to storing the literal color string for the rare case where a pixel's color isn't in the palette (e.g. a leftover color from before a palette change), and transparently decodes links generated by older versions of the app (which always used literal color strings) — no version bump or migration needed.
- Corrected the QR success message, which still described the old (removed in v1.2.5) auto-download behavior.
- The eyedropper (especially the whole-screen picker added in v1.2.4) could pick up a color that isn't one of the current palette's 16/64/256 colors — e.g. sampling an arbitrary photo on screen while CGA is selected. New `findNearestColor()` (`logic.js`) finds the closest color in the active palette (by Euclidean RGB distance) and the eyedropper now always applies that instead of the raw picked color, so the drawing stays consistent with the palette you chose instead of quietly accumulating off-palette colors. Shown once per session, the first time the eyedropper is used, an alert explains that colors get matched to the closest one in the current palette.

## v1.2.7 - VGA palette: spread out the filler colors

- **Bug fix**: the fix that made `generateVGAColors()` return 256 truly distinct colors (v1.2.1) filled the ~18 slots the CGA/grayscale/web-safe families fall short of using whichever candidates it found first in scanning order — which, in practice, clustered them all in one small corner of RGB space, several visually indistinguishable from each other and from already-present colors (e.g. `rgb(17,17,17)` next to `rgb(25,25,25)`, only 8 apart per channel). New `pickFarthestColors()` (`logic.js`, pure, tested) picks the filler colors greedily by maximum distance from every color already in the palette (and from each other, as they're picked) instead. The closest pairs in the resulting palette are now the ones inherent to combining CGA's 85-step and web-safe's 51-step spacing (e.g. `rgb(170,0,0)`/`rgb(153,0,0)`, 17 apart) — not fixable without abandoning the historical CGA/web-safe structure, and arguably authentic to how real VGA palettes looked.

## v1.2.8 - Authentic CGA/EGA/VGA palettes (superseding v1.2.7's VGA fix)

Prompted by a question about whether the VGA palette matched real Mode 13h hardware. It didn't, in two different ways — one already-correct-looking generator (EGA) turned out to have the right 64 colors in the wrong order, and the VGA "fix" from v1.2.1–v1.2.7 (always exactly 256 distinct colors) turned out to be optimizing for the wrong goal once the real reference was found.

- **`generateVGAColors()` replaced entirely** with the literal, byte-accurate default palette of VGA "Mode 13h" — the same 256 colors real VGA hardware/BIOS loads for that mode. Values extracted mechanically (no manual transcription) from `VGA_palette_with_black_borders.svg` on Wikimedia Commons, the reference image linked from Wikipedia's [Mode 13h](https://en.wikipedia.org/wiki/Mode_13h) article. This palette has only **244 distinct colors, not 256** — and that's correct, not a bug: index 16 (start of the grayscale ramp) is black, several ramp values coincide with the first 16 CGA colors, and indices 248–255 are all black (reserved/unused on real hardware). This supersedes v1.2.1's and v1.2.7's approach of guaranteeing 256 always-distinct colors, which turned out to be solving a self-imposed problem rather than matching the real palette; `pickFarthestColors()` (added in v1.2.7) is removed, no longer needed.
- **`generateEGAColors()` reworked**: the 64 colors themselves were already correct (all combinations of 4 levels per channel), but the *order* was a plain nested r/g/b loop, not the real hardware order. EGA's palette registers use a 6-bit "rgbRGB" format (high 3 bits = low-intensity/+85 per channel, low 3 bits = high-intensity/+170 per channel, summed), documented in Wikipedia's [Enhanced Graphics Adapter](https://en.wikipedia.org/wiki/Enhanced_Graphics_Adapter) article with citations to IBM's original EGA manual. Rewrote the generator around this exact bit formula, and cross-checked the result two ways: against the article's documented CGA-compatible 16-color index table (index 0,1,2,3,4,5,7 and notably **20** for brown — not the 6 a linear reading would suggest — through 56–63), and against `EGA64_Full_Palette.png` (Wikimedia Commons, pixel-sampled programmatically) — both matched exactly.
- **`generateCGAColors()` double-checked, unchanged**: its 16 values already matched both the VGA reference's first 16 entries and the EGA article's CGA-compatible index table exactly, byte for byte.
