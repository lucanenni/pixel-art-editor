...
Three generator functions (in `logic.js`, unit-tested — see "File split" above), each returning an array of `rgb(r,g,b)` strings. Since v1.2.8 all three match real historical hardware exactly, not just "CGA/EGA/VGA-inspired" approximations — see the sources cited in each function's comment in `logic.js`, and in `CHANGELOG.md`'s v1.2.8 entry.

- `generateCGAColors()` — 16 hardcoded colors, the standard IBM CGA palette. Cross-checked against both sources below (it's the first 16 entries of the VGA table, and matches the EGA article's CGA-compatible index table).
- `generateEGAColors()` — 64 colors in **real hardware register order**: for index `i` (0–63), 3 "low" bits (bits 5-3, contributing +85 per channel if set) and 3 "high" bits (bits 2-0, contributing +170 per channel if set) are decoded and summed per channel — the "rgbRGB" register format documented in Wikipedia's [Enhanced Graphics Adapter](https://en.wikipedia.org/wiki/Enhanced_Graphics_Adapter) article, citing IBM's original EGA manual. This isn't just "the right 64 colors" (a naive nested r/g/b loop over the same 4 levels would give that) — it's specifically the order real EGA palette registers use, which puts brown at index 20, not a "linear" 6. Verified against the article's CGA-compatible 16-color index table and, independently, by pixel-sampling `EGA64_Full_Palette.png` (Wikimedia Commons) programmatically — both matched the formula exactly.
- `generateVGAColors()` — the literal 256-color default palette of VGA "Mode 13h", hardcoded as a flat array (16 colors × 16 rows, matching the reference image's layout, with a comment per row). Extracted mechanically (a throwaway script parsed the SVG's `fill` attributes in order — no manual transcription of 256 values) from `VGA_palette_with_black_borders.svg` (Wikimedia Commons), the image linked from Wikipedia's [Mode 13h](https://en.wikipedia.org/wiki/Mode_13h) article.
  - **Only 244 of the 256 are distinct — this is correct, not a bug**: index 16 (start of the grayscale ramp) is black same as index 0, a few ramp values coincide with the first 16 CGA colors, and indices 248–255 are all black (reserved/unused on real hardware). A test asserts exactly this (244 distinct, indices 16/31/248-255 at their expected values), rather than asserting "256 distinct" the way earlier versions did.
  - **History**: an early version (until v1.2.1) padded a shortfall of *invented* colors by repeating plain black, so `rgb(0,0,0)` accounted for 11 of 256 slots — that was a real bug, since the whole palette wasn't sourced from anywhere real yet. v1.2.1 and v1.2.7 then went in the direction of guaranteeing 256 always-*distinct* colors (first via ad hoc filler colors, then via a `pickFarthestColors()` greedy-spread helper) — a reasonable goal on its own, but it turned out to be solving self-imposed problem once the real Mode 13h reference was found: the authentic palette was never meant to have 256 distinct colors. v1.2.8 replaced the whole generator with the literal reference table and removed `pickFarthestColors()`, no longer needed.
...
Since v1.2.4, clicking the eyedropper button calls `activateEyedropper()` (`async`), which first shows a one-time-per-session `alert()` (guarded by `eyedropperNoticeShown`) explaining that the picked color gets matched to the current palette (see `pickColor()` below), then branches on whether the browser supports the native [`EyeDropper` API](https://developer.mozilla.org/en-US/docs/Web/API/EyeDropper) (`window.EyeDropper` — Chromium-based browsers only, not Firefox/Safari):

- **Supported**: `new EyeDropper().open()` hands the *entire screen* over to the browser's own picker UI (not limited to this page, or even this browser window) and resolves with `{ sRGBHex: "#rrggbb" }`. The result is converted with `hexToRGB()` and passed to `pickColor()`. If the user cancels (e.g. `Esc`) the promise rejects; the `catch` swallows it silently — there's nothing to recover, the user just didn't pick anything. Either way (`try` or `catch`) it finishes by calling `setTool("brush")`, exactly like the fallback path below.
- **Not supported**: falls back to `setTool("eyedropper")`, entering the same "click a canvas cell to pick its color" mode as before v1.2.4 — this is the branch in `handleDraw()` under `currentTool === "eyedropper"`. An undrawn cell now resolves to `"rgb(255,255,255)"` (the same white as the visible background) instead of doing nothing, so a click always picks *something*; either way the color goes through `pickColor()`, not `selectColor()` directly.
...
`pickColor(rawColor)` (v1.2.6) is what actually applies a picked color: `selectColor(findNearestColor(rawColor, generateColors()))`. Both eyedropper paths can produce a color outside the current palette — trivially with the whole-screen picker (any color on screen, any palette selected), but even the canvas-only fallback could in principle hit a leftover off-palette pixel (e.g. after a palette change). `findNearestColor()` (`logic.js`, pure, by Euclidean RGB distance) keeps the drawing consistent with the 16/64/256 colors of whichever palette is active, rather than quietly letting arbitrary colors in. Plain swatch clicks still go through `selectColor()` directly — they're always already exactly a palette color, so snapping would be a no-op there.
...
`showQRCode()`:
1. builds the compact object `{ g: gridSize, p: currentPalette, d: encodePixelsCompact(pixels, currentPalette) }` — `d` stores each pixel as a **palette index** (a small number) rather than a full `"rgb(r,g,b)"` string, since CGA/EGA/VGA only ever have 16/64/256 possible colors; see `encodePixelsCompact()`/`decodePixelsCompact()` in `logic.js`. This mattered in practice: before this (v1.2.6), an 8x8 grid — the *smallest* grid size — fully painted could already exceed the URL length limit below, which was confusing since "try a smaller grid" isn't actionable when you're already on the smallest one.
...
On page load, `loadSharedDrawingFromURL()` checks for a `?data=` query parameter: if present, it decodes it, validates `g`/`p` the same way `importDrawing()` does (`VALID_GRID_SIZES`, `VALID_PALETTES` — the URL is just as untrusted as a hand-edited import file), converts `d` back from palette indices to `"rgb(r,g,b)"` strings with `decodePixelsCompact(compactData.d, compactData.p || currentPalette)`, then runs the result through the same `filterValidPixels()` as everywhere else, calls `clearHistory()` (a freshly loaded drawing has no undo history), restores the state (grid/palette/pixels), redraws it, calls `saveState()`, cleans up the URL with `history.replaceState` (so a later page refresh doesn't re-trigger this), and shows an `alert()` pointing the user at the "Scarica immagine" button. It returns `true` if it actually loaded something, `false` otherwise (no `?data=`, or invalid data) — the startup sequence uses this to decide whether to fall back to the autosave (see below).

`decodePixelsCompact()` also transparently handles links generated by older versions of the app, which stored literal `"rgb(...)"` strings in `d` instead of indices — it passes a string value through unchanged, only looking up a palette color when the value is a `number`. No version bump or migration was needed for this.

⚠️ Historical behavior (removed in v1.2.5): this used to call `downloadImage()` automatically inside a `setTimeout`, right after loading. Browsers increasingly block a download triggered this way — by script, with no direct/fresh user gesture behind it — and they do it *silently* (no exception, nothing to detect), so in practice the automatic download worked inconsistently with no feedback explaining why. Removed in favor of pointing the user at a real button click, which doesn't have this problem. If you're tempted to re-add an automatic side effect like this (auto-download, auto-copy-to-clipboard, etc.) triggered by a page load rather than a direct click/tap, assume it will be silently blocked in some browsers and design for that from the start.

**Known limitation**: a standard QR code's capacity is limited; drawings with many colored pixels (especially on large grids) can still exceed the threshold even with the compact index encoding above, and fall back to the "metadata only" case. There is currently no fallback that still loads the drawing in that case — the user is pointed to `Esporta JSON` instead.
