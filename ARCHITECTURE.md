...
- `generateVGAColors()` — 256 colors: 16 CGA colors + 16 grays + 216 web-safe colors (6 levels per channel, 6³ = 216). These three families overlap (e.g. black and white appear in more than one of them), so they only yield ~238 *distinct* colors on their own; a `pushUnique()` helper skips anything already seen. The remaining ~18 slots are filled by `pickFarthestColors()` (`logic.js`, pure, tested — see "File split" above) from a denser candidate grid (levels `[16,48,80,112,144,176,208,240]`, chosen to not exactly coincide with the CGA/grayscale/web-safe values above), picking greedily by **maximum distance** from every color already in the palette (and from each other, as they're picked), so the palette always has exactly 256 distinct colors *and* the filler ones aren't visually clustered together.
  - An earlier version (until v1.2.1) padded the shortfall by repeating plain black, so `rgb(0,0,0)` alone accounted for 11 of the 256 slots.
  - The version right after that (v1.2.1–v1.2.6) picked filler colors from a small fixed grid in plain scanning order — mathematically distinct from everything else, but all ~18 of them ended up sharing nearly the same red/green component (since only the first ~18 combinations out of 125 in nested-loop order were ever needed), which looked like a cluster of near-duplicate colors (e.g. `rgb(17,17,17)` next to `rgb(25,25,25)`, 8 apart per channel) even though no two were ever technically identical. Fixed in v1.2.7 by `pickFarthestColors()`.
  - Some visually close (not identical) pairs remain and are expected: CGA's 85-step spacing and web-safe's 51-step spacing mathematically land only 17 apart in places (e.g. `rgb(170,0,0)`/`rgb(153,0,0)`) — inherent to combining these two historical systems, not something `pickFarthestColors()` touches (it only picks the *filler* colors, not the CGA/web-safe ones), and arguably authentic to how real VGA default palettes looked.
